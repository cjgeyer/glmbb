...
are irrelevant).
By ``term'' here we mean what the terms object is calling a terms,
corresponding to columns of the \texttt{"factors"} attribute (if any)
or to components of the \texttt{term.labels} attribute.
