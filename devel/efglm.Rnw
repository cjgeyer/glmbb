...
that make each model an exponential family is used.  Thus the
...
The object returned is a list having class \code{"efglm"}
...
If a DOR $\delta$ exists that is not a DOC, then then we can rescale $\delta$
so that its most negative component of $\eta = M \delta$ is equal to $-1$.
For this $\delta$ the objective function is equal to $-1$.
...
model is ``mulltinomial'' rather than ``product multinomial''.
\begin{proof}
...
Suppose the indicator vectors $u_A$ having components $u_{A, i}$ given by
$$
   u_{A, i} = \begin{cases} 1, & i \in A \\ 0 & i \in I \setminus A \end{cases}
$$
...
When the MLE does not exist in Poisson GLM, any GDOR for this model
