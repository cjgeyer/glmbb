...
\section{Using CRAN Package Rcdd}
...
\section{Digression about Information Criteria}
...
BIC(gout.lcm)
foo <- logLik(gout)
- 2 * as.numeric(foo) + log(length(gout$y)) * attr(foo, "df")
- 2 * as.numeric(foo) + log(sum(gout$y)) * attr(foo, "df")
foo <- logLik(gout.lcm)
- 2 * as.numeric(foo) + log(length(gout.lcm$y)) * attr(foo, "df")
...
\section{Using CRAN Package ClpAPI}
...
Now we need to recall our linear program \eqref{prog:mu-hat}, and
recall that the model matrix is R object \code{m}, the conditions
about $\hat{\mu}$ in \eqref{prog:mu-hat} are coded in R object \code{is.zero},
and the objective vector is R object \code{objgrd}.  We start with the latter.
This seems to need to be a dense vector.
<<clpAPI-objective>>=
chgObjCoefsCLP(lp, objgrd)
@
Now for our constraints.  We put in the coefficients matrix as a sparse matrix
one row at a time.
<<clpAPI-constraints>>=
for (i in 1:nrow(m)) {
    lb <- if (is.zero[i]) -1 else 0
    m.is.nonzero <- m[i, ] != 0
    vals <- m[i, m.is.nonzero]
    # - 1 here for zero-origin indexing (going to C++)
    cols <- which(m.is.nonzero) - 1
    addRowsCLP(lp, 1, lb, 0, c(0, length(vals)), cols, vals)
}
@
Now for the column bounds (the defaults are wrong for our problem).
<<clpAPI-column-bounds>>=
chgColLowerCLP(lp, rep(-Inf, ncol(m)))
chgColUpperCLP(lp, rep(Inf, ncol(m)))
@
...
primalCLP(lp)
getSolStatusCLP(lp)
delta.other <- as.numeric(delta)
delta <- getColPrimCLP(lp)
all.equal(delta, delta.other)
@
